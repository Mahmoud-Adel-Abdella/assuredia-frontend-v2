...
if (
  !fs.existsSync(
    path.join(
      ...
      "main",
      "resources",
      "migration-015-test-definition-persistence.sql",
    ),
  )
) {
  console.error(
    "ASSUREDIA_BACKEND_DIR does not look like the automation-engine repository (migration 015 missing).",
  )
  process.exit(2)
}
if (
  !fs.existsSync(
    path.join(
      ...
      "main",
      "resources",
      "migration-016-test-creation-requests.sql",
    ),
  )
) {
  console.error(
    "ASSUREDIA_BACKEND_DIR does not include the PR10A foundation (migration 016 missing).",
  )
  process.exit(2)
}
if (
...
function run(cmd, args, opts = {}) {
  const child = spawn(cmd, args, {
    stdio: opts.pipe ? ["ignore", "pipe", "pipe"] : "ignore",
    ...opts,
  })
  return child
}
...
function runCapture(cmd, args, opts = {}) {
  ...
  if (res.status !== 0) {
    throw new Error(
      `${cmd} ${args.join(" ")} failed (${res.status}): ${(res.stderr || res.stdout || "").slice(0, 500)}`,
    )
  }
  return res.stdout
}
...
async function main() {
  ...
  try {
    ...
    console.log(
      "[2/7] applying bootstrap + migrations through the engine's ledger",
    )
    fs.mkdirSync(path.join(work, "resources"), { recursive: true })
    fs.cpSync(
      path.join(BACKEND_DIR, "tools", "postgres-bootstrap.sql"),
      path.join(work, "000-bootstrap.sql"),
    )
    fs.cpSync(
      path.join(BACKEND_DIR, "src", "main", "resources"),
      path.join(work, "resources"),
      { recursive: true },
    )
    runCapture("docker", [
      "cp",
      path.join(work, "000-bootstrap.sql"),
      `${containerName}:/tmp/000-bootstrap.sql`,
    ])
    runCapture("docker", [
      "cp",
      path.join(work, "resources"),
      `${containerName}:/tmp/resources`,
    ])
    ...
    for (
      let migrationAttempt = 1;
      migrationAttempt <= 5 && !migrated;
      migrationAttempt++
    ) {
      try {
        runCapture("docker", [
        ...
        lastMigrationError =
          ...
          " | stdout=" +
          String(
            err && err.stdout ? String(err.stdout).slice(0, 200) : "(none)",
          )
        console.log(
          "  migration attempt " +
            migrationAttempt +
            " failed: " +
            lastMigrationError.slice(0, 300),
        )
    ...
    if (!migrated)
      throw new Error(
        "Migration application failed after retries: " + lastMigrationError,
      )
    const ledger = runCapture("docker", [
      "exec",
      containerName,
      "psql",
      "-U",
    ...
    console.log(
      "[3/7] seeding synthetic fixtures (pgcrypto bcrypt, per-run password)",
    )
    ...
    const clientAId = Number(
      runCapture("docker", [
        ...
        "-tA",
        "-c",
        "SELECT id FROM clients WHERE client_name = 'live-sandbox-alpha'",
      ]).trim(),
    )
    const flowAId = Number(
      runCapture("docker", [
        "exec",
        containerName,
        "psql",
        ...
        "-tA",
        "-c",
        "SELECT id FROM flows WHERE flow_name = 'live-checkout-flow'",
      ]).trim(),
    )

    /* ---- 4. backend jar ---- */
    console.log(
      "[4/7] building the backend jar (skipTests; the full suite is a separate gate)",
    )
    ...
    const mvnArgs = isWin
      ? ["/c", "mvn", "-B", "clean", "package", "-DskipTests"]
    ...
    console.log(
      "[5/7] starting the loopback fixture site on 127.0.0.1:" + sitePort,
    )
    fs.writeFileSync(
    ...
    const backend = spawnTracked(
      ...
      [
        "-jar",
        path.join(BACKEND_DIR, "target", "TestNG_Framework-1.0-SNAPSHOT.jar"),
      ],
      {
        cwd: BACKEND_DIR,
        env: {
          ...
          SYNC_CLIENTS_ROOT:
            path.join(work, "runtime-clients").replace(/\\/g, "/") + "/",
    ...
    console.log(
      "  warm-up executions (absorb Playwright Java first-use costs; deleted afterwards)",
    )
    ...
    const warmDef = await api(
      "POST",
      `/dashboard-api/clients/${clientAId}/test-definitions`,
      { name: "warmup-def", initialSourceJson: warmSource },
      adminToken,
    )
    if (warmDef.status !== 200)
      throw new Error(
        "Warm-up definition creation failed: " +
          warmDef.status +
          " " +
          JSON.stringify(warmDef.json),
      )
    await api(
      "PUT",
      `/dashboard-api/clients/${clientAId}/test-definitions/${warmDef.json.definitionId}`,
      {
        name: "warmup-def",
        flowId: flowAId,
      },
    ...
    const validated = await api(
      "POST",
      `/dashboard-api/clients/${clientAId}/test-definitions/${warmDef.json.definitionId}/versions/${warmDef.json.initialVersionId}/validate`,
      undefined,
      adminToken,
    )
    if (validated.json.status !== "VALIDATED")
      throw new Error(
        "Warm-up validation failed: " + JSON.stringify(validated.json),
      )
      ...
      if (warmRun.json.status === "PASSED") {
        ...
        try {
          const row = await psql(
            db,
            `SELECT COALESCE(error_message, '(null)') FROM test_runs WHERE implementation_type = 'TEST_DEFINITION' ORDER BY id DESC LIMIT 1`,
          )
          diag = row.trim()
        } catch {
          diag = "(db probe failed)"
        }
        console.log(
          `  warm-up attempt ${attempt}: ${warmRun.json.status}/${warmRun.json.terminatingReasonCode || "?"} — ${diag}`,
        )
      }
    ...
    if (!warmPassed)
      throw new Error(
        `Warm-up trial never passed after ${WARMUP_ATTEMPTS} attempts — the worker environment is not healthy enough for the audited run`,
      )
    ...
    /* ---- frontend ---- */
    console.log(
      "[7/7] starting the real frontend dev server on 127.0.0.1:" +
        frontendPort,
    )
    ...
    console.log(
      "running the Playwright real-stack suite (real Chromium, headed=" +
        HEADED +
        ")",
    )
    playwright = spawn(
      ...
      [
        ...
        "test",
        "-c",
        "e2e/live/playwright.live.config.ts",
      ],
      {
        cwd: repoDir,
        stdio: "inherit",
          ...
          LIVE_CLIENT_B_FLOW_ID: String(
            Number(
              runCapture("docker", [
                ...
                "-tA",
                "-c",
                "SELECT id FROM flows WHERE flow_name = 'live-beta-flow'",
              ]).trim(),
            ),
          ),
    ...
    if (!(process.env.KEEP_ON_FAILURE === "1" && exitCode !== 0)) {
      try {
        runCapture("docker", ["stop", containerName])
      } catch {}
      try {
    ...
    console.log(
      "cleanup complete (container, processes, temporary files removed)",
    )
  }
