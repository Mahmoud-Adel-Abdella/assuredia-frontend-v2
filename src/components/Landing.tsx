...
function IconX({ className }: { className?: string }) {
  return <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
}
...
function IconGlobe({ className }: { className?: string }) {
  return <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><line x1="2" y1="12" x2="22" y2="12" /><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z" /></svg>
}
function IconShield({ className }: { className?: string }) {
  return <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>
}
...
/* ------------------------------------------------------------------ */
const features = [
  { Icon: IconLineChart, title: "24/7 Monitoring", description: "Keep critical flows under continuous watch, around the clock." },
  { Icon: IconGlobe, title: "UI + API Coverage", description: "Monitor both user journeys and backend APIs from a single platform." },
  { Icon: IconShield, title: "Zero-Code Intrusion", description: "Connect your application and monitor critical flows without changing your source code." },
  { Icon: IconBrain, title: "AI-Powered Insights", description: "Turn failures into useful context for faster investigation and response." },
]
...
/* Process flow                                                        */
/* ------------------------------------------------------------------ */
function BrowserTile({ variant = "test", status = "pass" }: { variant?: "test" | "action"; status?: "pass" | "fail" }) {
  return (
    <div className="relative h-[88px] w-[110px] rounded-[8px] border border-white/[0.09] bg-[rgba(15,20,32,0.70)] backdrop-blur-sm shadow-sm">
      ...
      {status === "fail" ? (
        <div className="absolute -bottom-[6px] -right-[6px] flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-red-500 to-rose-600 text-white shadow-[0_2px_8px_rgba(239,68,68,0.45)]">
    ...
    <div className="relative mx-auto flex w-full max-w-[780px] items-center justify-center overflow-visible">
      ...
      <div className="grid w-full grid-cols-4 items-stretch gap-4">
        <div className="relative">
          <GlassStepCard number="1" title="Monitor" description="UI & API flows watched continuously, 24/7" style={au(660)}>
            <BrowserTile status="fail" />
          </GlassStepCard>
          <ConnectorArrow />
        </div>
        <div className="relative">
          <article
            className={`anim-enter group relative z-10 flex min-h-[260px] shrink-0 flex-col rounded-[14px] p-6 transition-all duration-300 hover:-translate-y-0.5 ${G.detect} hover:border-white/[0.20]`}
            style={au(780)}
          >
            <div className="flex items-center gap-2.5">
              <span className="flex h-[26px] w-[26px] items-center justify-center rounded-[7px] bg-gradient-to-br from-[#2dd4bf] to-[#0fa89b] text-[12px] font-bold text-white shadow-[0_2px_6px_rgba(45,212,191,0.35)]">2</span>
              <h3 className="text-[15px] font-bold tracking-[-0.01em] text-navy">Detect</h3>
            </div>
            <p className="mt-3 text-[12px] font-medium leading-[1.6] text-slate-400">Catch failures the moment they happen.</p>
            <div className="mt-auto pb-[23px]" />
        ...
        <div className="relative">
          <GlassStepCard number="3" title="Inform" description="Get instant alerts with the context your team needs." glassClass={`${G.inform} overflow-hidden`} style={au(900)}>
            <div className="my-[9px] flex h-[79px] w-fit flex-col items-center gap-3">
              <div className="relative flex h-[79px] w-[79px] items-center justify-center rounded-full bg-brand-50/60">
                <IconBell className="size-[35px] text-brand-400" />
                <span className="absolute -right-1 top-0 flex h-[32px] w-[32px] items-center justify-center rounded-full bg-error text-[11px] font-bold text-white shadow-[0_2px_8px_rgba(239,68,68,0.4)]">1</span>
              </div>
            </div>
          </GlassStepCard>
          <ConnectorArrow />
        ...
        <div className="relative">
          <GlassStepCard number="4" title="Action" description="Understand the issue and act faster." style={au(1020)}>
  ...
  return (
    <div className={`mx-auto mt-5 flex h-[52px] w-full max-w-[700px] items-center justify-between gap-5 rounded-[12px] px-6 text-navy ${G.status}`}>
      ...
      <span className="text-[13px] font-semibold">
        <span className="text-[#2dd4bf]">Always On</span>
        <span className="ml-1 font-medium text-slate-400">Monitoring</span>
      </span>
    </div>
  )
}
...
function BusinessValue() {
  const { ref, visible } = useInView()
  return (
    <section className="relative z-10 mx-auto w-full max-w-[1400px] px-6 pt-4">
      <div
        ref={ref}
        className={`anim-enter rounded-[14px] px-8 py-6 ${G.panel}`}
        style={visible ? au(80) : { opacity: 0 }}
      >
        <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#2dd4bf]">Always On. Quality Assured.</p>
        <h2 className="mb-3 font-display text-[20px] font-bold text-navy">Catch Problems Before Your Users Do.</h2>
        <p className="max-w-[680px] text-[13px] leading-6 text-slate-400">
          Assuredia continuously watches the flows that matter to your business, so your team can detect failures earlier and respond before they become production surprises.
        </p>
      </div>
    </section>
  )
    ...
    >
      <div className={`grid overflow-hidden rounded-[14px] md:grid-cols-[170px_1fr] ${G.panel}`}>
        <div className="flex items-center border-b border-white/[0.07] px-6 py-5 md:border-b-0 md:border-r">
          <h2 className="text-[15px] font-extrabold leading-7 text-navy">
            Built for<br />Reliable<br />Releases
          </h2>
        </div>
      ...
      <div className={`relative overflow-hidden rounded-[14px] px-7 py-6 ${G.panel}`}>
        <h2 className="relative z-10 max-w-[360px] text-[14px] font-normal leading-6 text-navy pr-[114px]">
          From critical user journeys to backend APIs, Assuredia keeps your quality checks running continuously—so your team can focus on shipping.
        </h2>
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background font-sans text-navy antialiased">
      ...
      <section className="relative min-h-screen pb-5 pt-[100px]">
        ...
        <div className="relative z-10 mx-auto grid max-w-[1400px] items-center gap-8 px-6 lg:grid-cols-[.72fr_1.28fr]">
          <div className="max-w-xl">
            ...
            <p className="anim-enter text-[15px] leading-relaxed text-slate-400" style={au(420)}>
              Continuously monitor your critical UI & API flows 24/7, detect failures instantly, and get the context you need to act.
            </p>
        ...
        <div className="relative z-10 mt-8 lg:hidden">
          <div className={`mx-6 rounded-[14px] p-5 ${G.card}`}>
            <div className="grid gap-3 sm:grid-cols-2">
              {["1. Monitor", "2. Detect", "3. Inform", "4. Action"].map((step) => (
        ...
        {/* Feature rail + business value + tech panel */}
        <div className="mt-10">
          <FeatureRail />
          <BusinessValue />
          <TechnologyPanel />
...
export const LandingGlass = Landing
